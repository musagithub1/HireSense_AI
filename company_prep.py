...
def get_company_specific_questions(
    company_key: str, interview_type: str = "Mixed"
) -> List[str]:
...
def get_faang_companies() -> List[Dict]:
    ...
    return [{"key": key, **COMPANY_PROFILES[key]} for key in FAANG_COMPANIES]


def get_top_tech_companies() -> List[Dict]:
    """Get top tech company profiles."""
    return [{"key": key, **COMPANY_PROFILES[key]} for key in TOP_TECH]
...
def compare_companies(company_keys: List[str]) -> Dict:
    ...
    comparison = {"companies": [], "common_focus": [], "unique_aspects": {}}
    ...
    comparison["common_focus"] = [
        area for area, count in focus_counter.items() if count > 1
    ]
