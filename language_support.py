...
def get_language_specific_tts_html(
    text: str, lang_code: str, speed: float = 1.1
) -> str:
    """Generate injection-safe browser-native TTS HTML."""
    text_json = json_for_script(text[:2_000])
    language_json = json_for_script(get_tts_language(lang_code))
    bounded_speed = max(0.5, min(2.0, float(speed)))
