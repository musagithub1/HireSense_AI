...
- Posture: Analyzes body positioning for visible posture indicators
...
POSTURE_THRESHOLDS = {"excellent": 85, "good": 70, "needs_improvement": 55, "poor": 0}
...
def analyze_filler_words(transcript: str, duration_seconds: float) -> FillerWordMetrics:
    ...
    worst_offenders = sorted(
        filler_breakdown.items(), key=lambda x: x[1], reverse=True
    )[:5]

    # Generate feedback
    feedback = _generate_filler_feedback(
        rating, filler_breakdown, filler_percentage, worst_offenders
    )
...
def identify_strengths_and_improvements(
    ...
    if eye_contact.rating in ["excellent", "good"]:
        strengths.append(
            f"Strong eye contact ({eye_contact.eye_contact_percentage:.0f}% engagement)"
        )
    else:
        improvements.append(
            f"Eye contact needs work ({eye_contact.eye_contact_percentage:.0f}% engagement)"
        )
    ...
    if posture.rating in ["excellent", "good"]:
        ...
        improvements.append(
            f"Posture could be improved (score: {posture.posture_score:.0f}/100)"
        )

    # Filler words
    if filler_words.rating in ["excellent", "good"]:
        strengths.append(
            f"Minimal filler words ({filler_words.filler_word_percentage:.1f}%)"
        )
    else:
        improvements.append(
            f"Reduce filler words ({filler_words.filler_word_percentage:.1f}% of speech)"
        )
...
def generate_detailed_analysis_report(
    analysis_result: NonVerbalAnalysisResult, interview_context: Optional[str] = None
) -> Iterator[str]:
    """
...
- Posture indicators: {", ".join(analysis_result.posture.posture_indicators) or "None detected"}
...
def get_nonverbal_analysis_component_html(
    video_blob_url: str, height: int = 600
) -> str:
    """
