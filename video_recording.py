    ...
    (function() {
        ...
        if (recordings.length > 0) {
            ...
            recordings.reverse().forEach((rec, index) => {
                ...
                title.textContent = `📹 Interview Recording ${recordings.length - index}`;
                const date = document.createElement('span');
                date.className = 'recording-date';
                const parsedDate = new Date(rec.timestamp);
                ...
                addStat(`⏱️ ${String(rec.duration || 'N/A')}`);
                addStat(`📍 ${Array.isArray(rec.markers) ? rec.markers.length : 0} markers`);
                addStat(`💾 ${String(rec.fileSize || 'N/A')}`);
